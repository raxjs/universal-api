...
const createContext = normalize((canvasOptions: Options): Promise<CanvasContext> => {
  ...
  return new Promise((resolve, reject) => {
    const canvasNode: HTMLCanvasElement = (document.getElementById(canvasId) as HTMLCanvasElement);
    if (!canvasNode) reject(new Error('The canvas node may not exist.'));
    const context: CanvasContext = canvasNode.getContext(type, options); // For fallback
