...
实现动画，通过 [weex-bindingx](https://www.npmjs.com/package/weex-bindingx) 环境允许的前提下优先使用 `bindingx`，不满足环境要求则使用 [@uni/transition](https://www.npmjs.com/package/@uni/transition) 调用Weex或浏览器或小程序提供的动画API实现。
